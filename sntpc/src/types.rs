...
pub trait NtpUdpSocket<'a> {
    ...
    /// available in `addr`
    async fn send_to<T: net::ToSocketAddrs>(
        &self,
        buf: &[u8],
        addr: T,
    ) -> Result<usize>;
    ...
    async fn recv_from(&self, buf: &mut [u8]) -> Result<(usize, net::SocketAddr)>;
}
