...
/// ```
pub async fn sntp_send_request<'a,A,U, T>(
    dest: A,
    socket: &'a U,
    context: NtpContext<T>,
...
/// }
/// ```
pub async fn sntp_process_response<'a,A, U, T>(
    dest: A,
    socket: &U,
    ...
    T: NtpTimestampGenerator + Copy,
{
    ...
    if response != mem::size_of::<NtpPacket>() {
        return Err(Error::IncorrectPayload1(response, mem::size_of::<NtpPacket>()));
    }
...
async fn send_request<'a,A: net::ToSocketAddrs, U: NtpUdpSocket<'a>>(
    dest: A,
    req: &NtpPacket,
    socket: &'a U,
...
fn debug_ntp_packet(packet: &NtpPacket, _recv_timestamp: u64) {
    ...
    debug!("| Receive timestamp   (client):\t{:?}", _recv_timestamp);
    debug!(
        "| Reference timestamp (server):\t{:?}",
